...
for(int i=0;i<locations.size();i++)
    cout<<locations[i]<<"\\steamapps\\appmanifest_291210.acf"<<endl;

string finallocation; //The string in which existence of uberstrike installation has to be checked
int count=0;
int flag=1;
	while(count>10)
    {
    ...
    if (isFileExistsAccess(finallocation.c_str()))
        {cout<<"Uberstrike is installed in:"<<endl<<finallocation;
         flag++;
        }
    else count=count+1;
    }
